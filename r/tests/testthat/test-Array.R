...
test_that("Array", {
  ...
  x_dbl <- arrow_array(c(1,2,3,4,5,6))
...
test_that("empty arrays are supported", {
  x <- character()
  expect_equal(arrow_array(x)$as_vector(), x)
...
test_that("arrow_array() supports the type= argument. conversion from INTSXP and int64 to all int types", {
  ...
  for(type in types) {
    expect_equal(arrow_array(num_int32, type = type)$type, type)
    expect_equal(arrow_array(num_int64, type = type)$type, type)
  }
})

test_that("arrow_array() aborts on overflow", {
  expect_error(arrow_array(128L, type = int8())$type, "Invalid.*downsize")
  expect_error(arrow_array(-129L, type = int8())$type, "Invalid.*downsize")

  expect_error(arrow_array(256L, type = uint8())$type, "Invalid.*downsize")
  expect_error(arrow_array(-1L, type = uint8())$type, "Invalid.*downsize")

  expect_error(arrow_array(32768L, type = int16())$type, "Invalid.*downsize")
  expect_error(arrow_array(-32769L, type = int16())$type, "Invalid.*downsize")

  expect_error(arrow_array(65536L, type = uint16())$type, "Invalid.*downsize")
  expect_error(arrow_array(-1L, type = uint16())$type, "Invalid.*downsize")

  expect_error(arrow_array(65536L, type = uint16())$type, "Invalid.*downsize")
  expect_error(arrow_array(-1L, type = uint16())$type, "Invalid.*downsize")

  expect_error(arrow_array(bit64::as.integer64(2^31), type = int32()), "Invalid.*downsize")
  expect_error(arrow_array(bit64::as.integer64(2^32), type = uint32()), "Invalid.*downsize")
})
...
test_that("arrow_array() does not convert doubles to integer", {
  ...
  for(type in types) {
    expect_error(arrow_array(10, type = type)$type, "Cannot convert.*REALSXP")
  }
})
