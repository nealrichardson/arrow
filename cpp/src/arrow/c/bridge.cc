...
struct ArrayImporter {
 ...
 protected:
  ...
  Status ProcessListLike() {
    ...
    RETURN_NOT_OK(ImportOffsetsBuffer(1, sizeof(typename ListType::offset_type)));
    return Status::OK();
  }
  ...
  Status ProcessUnion() {
    ...
    RETURN_NOT_OK(AllocateArrayData(type));
    RETURN_NOT_OK(ImportNullBitmap());
    RETURN_NOT_OK(ImportFixedSizeBuffer(1, sizeof(uint8_t)));
  ...
  Status ImportStringLike(const std::shared_ptr<DataType>& type) {
    ...
    RETURN_NOT_OK(AllocateArrayData(type));
    RETURN_NOT_OK(ImportNullBitmap());
    RETURN_NOT_OK(ImportOffsetsBuffer(1, sizeof(OffsetType)));
  ...
  Status MakeChildField(int64_t child_id, std::shared_ptr<Field>* out) {
    // TODO metadata
    const ArrayImporter& child = *child_importers_[child_id];
    const ArrowArray* child_struct = child.c_struct_;
    if (child_struct->name == nullptr) {
      return Status::Invalid("Expected non-null name in imported array child");
    }
    bool nullable = (child_struct->flags & ARROW_FLAG_NULLABLE) != 0;
    *out = field(child_struct->name, child.data_->type, nullable);
    return Status::OK();
  }
  ...
  Status ImportNullBitmap(int64_t buffer_id = 0) {
  ...
  Status ImportBitsBuffer(int64_t buffer_id) {
  ...
  Status ImportUnusedBuffer(int64_t buffer_id) { return ImportBuffer(buffer_id, 0); }

  Status ImportFixedSizeBuffer(int64_t buffer_id, int64_t byte_width) {
    // Compute visible size of buffer
    int64_t buffer_size = byte_width * (c_struct_->length + total_offset_);
    return ImportBuffer(buffer_id, buffer_size);
  }

  Status ImportOffsetsBuffer(int64_t buffer_id, int64_t byte_width) {
    // Compute visible size of buffer
    int64_t buffer_size = byte_width * (c_struct_->length + total_offset_ + 1);
    return ImportBuffer(buffer_id, buffer_size);
  }

  template <typename OffsetType>
  Status ImportStringValuesBuffer(int64_t offsets_buffer_id, int64_t buffer_id,
  ...
  Status ImportBuffer(int64_t buffer_id, int64_t buffer_size) {
